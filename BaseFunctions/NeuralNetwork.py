...
class Neuron:
    ...
    def activate(self, input : list):
        ...
        self.input = input
        # W: (H x I), a: (I x B)
        z_linear = _math.dot_product(input, self.weight)  # (H x B)
        
        S = len(z_linear)
        
        b_row = [row[0] for row in self.bias]  # (H x B)
        
        b_tiled = [b_row for _ in range(S)]

        self.z = _math.matrix_addition(b_tiled, z_linear)
